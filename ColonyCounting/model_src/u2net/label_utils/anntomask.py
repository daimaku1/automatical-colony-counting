...
def save_colored_mask(mask, save_path):
    lbl_pil = Image.fromarray(mask.astype(np.uint8), mode="L").convert('L')
...
def main(args):
    ...
    for imgId in tqdm.tqdm(imgIds, ncols=100):
        ...
        if len(annIds) > 0:
            ...
            save_colored_mask(mask.astype('uint8'), seg_output_path)


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", default="C:\dsb18_DUTS", type=str,
