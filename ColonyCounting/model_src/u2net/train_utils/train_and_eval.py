...
def criterion(inputs, target, weight=None):
    ...
    weight[torch.where(target >= 0.5)] = 10
    weight[torch.where(target < 0.5)] = 1
    losses = [F.binary_cross_entropy_with_logits(inputs[i], target, weight=weight) for i in range(len(inputs))]
