...
def main(args):
    ...
    train_dataset = DUTSDataset(args.data_path, train=True, transforms=SODPresetTrain([1280, 961], crop_size=1280))
    val_dataset = DUTSDataset(args.data_path, train=False, transforms=SODPresetEval([1280, 961]))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs - 1:
            ...
            if current_mae >= mae_info and current_f1 <= f1_info:
                torch.save(save_file, f"{args.save_path}/model_best.pth")

        # only save latest 40 epoch weights
        if os.path.exists(f"{args.save_path}/model_{epoch-40}.pth"):
            os.remove(f"{args.save_path}/model_{epoch-40}.pth")

        torch.save(save_file, f"{args.save_path}/model_{epoch}.pth")
...
def parse_args():
    ...
    parser.add_argument('--save-path', default='./save_weights', type=str)
