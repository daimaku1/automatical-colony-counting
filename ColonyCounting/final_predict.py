...
from model_src.u2net.src import u2net_full
from torchvision.models import resnet50

from model_src.u2net.pred_api import main as u2net_pred
from model_src.resnet__50.pred_api import main as resnet_pred
...
def image_process(imgs, masks, revolve=True):
    ...
    for img, mask in zip(imgs, masks):
        ...
        for label, (x, y, w, h, s) in enumerate(stats):
            ...
            seg_imgs.append(segment)
            label_imgs.append(img)
    return label_imgs, seg_imgs
...
def main():
    ...
    for img_path in img_paths:
        ...
        label_imgs, seg_imgs = image_process(imgs, masks, revolve=True)
        preds = resnet_predict(seg_imgs, r,)
        img_name = 'seg_' + os.path.split(img_path)[-1]
        save_res(save_dir, seg_imgs, [f'{preds[i]}_{i}_' + img_name for i in range(len(seg_imgs))])
        # save_res(save_dir, label_imgs, [img_name])
